...
function Main()
   ...
   local oWnd := TWindow():New(), oDatePck
   local dDate
   local oLabel
   
   // Month should be upper case (???)
   SET DATE FORMAT "MM-dd-yyyy"
   
   dDate := CToD( "01-01-2010" )
   
   oDatePck = TDatePicker():New( oWnd, 15, 0, bSETGET( dDate ) )
   
   oDatePck:bChanged = { | oSelf | oLabel:SetText( DToC( oDatePck:dValue ) ) } 
      
   oLabel = TLabel():New( oWnd, DToC( oDatePck:dValue ), 250, 100, 100, 50 )
