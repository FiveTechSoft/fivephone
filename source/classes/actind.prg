...
CLASS TActivityIndicator FROM TView
   
   METHOD New( oWnd ,ntop,nleft,nWidth,nHeight,nStyle )
   
   METHOD Start() INLINE ActIndStart( ::hWnd )
   
   METHOD Stop() INLINE ActIndStop( ::hWnd )
   
   METHOD SetStyle(nstyle) INLINE  ActIndStyle( ::hWnd,nStyle ) 
     
   METHOD IsAnimating() INLINE ActIndAnimating( ::hWnd )
   
ENDCLASS

METHOD New( oWnd,ntop,nleft,nWidth,nHeight,nstyle ) CLASS TActivityIndicator

DEFAULT nStyle := 3 ,nleft:=50,ntop:=50,nWidth:=50,nHeight:= 50

   ::hWnd = CreateActIndicator( oWnd:hWnd,ntop,nleft,nWidth,nHeight,nStyle )
