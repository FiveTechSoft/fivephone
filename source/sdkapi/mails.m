...

{
  ...
  NSString * cSubject = [ [ [ NSString alloc ] initWithCString: ISCHAR( 2 ) ? hb_parc( 2 ) : "" ] autorelease ];
  NSString * cMessageBody = [ [ [ NSString alloc ] initWithCString: ISCHAR( 3 ) ? hb_parc( 3 ) : "" ] autorelease ];
 
  MailComposerViewController *mandamail = [[MailComposerViewController alloc] init];
  [ window addSubview: mandamail.view ];
  
	if ([MFMailComposeViewController canSendMail])
	{
		MFMailComposeViewController *mailController = [[MFMailComposeViewController alloc] init];
		mailController.mailComposeDelegate = mandamail ;
		[mailController setSubject: cSubject ];
		[mailController setMessageBody: cMessageBody isHTML:NO];
		[mandamail presentModalViewController:mailController animated:YES];
		[mailController release];
	}	
	else {
		MsgInfo(@"No puede enviar el email");
	}
