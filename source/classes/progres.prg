...
CLASS TProgress FROM TView
     
   METHOD New( oWnd, nTop, nLeft, nWidth, nHeight, nPorcent )
   METHOD Set(nPorcent) INLINE SETPROGRESS(::hWnd, nPorcent ) 
   METHOD Resources( oWnd, idResource, nPorcent )
   
ENDCLASS

METHOD New( oWnd, nTop,nLeft,nWidth, nHeight, nPorcent ) CLASS TProgress

   DEFAULT nTop:=79, nLeft:=199, nWidth:=150, nHeight := 9 , nPorcent := 0    
   
   ::hWnd =  CreateProgress(oWnd:hwnd, nTop,nLeft,nWidth, nHeight )
   ::Set(nPorcent)
 
 return Self


METHOD Resources( oWnd, idResource, nPorcent ) CLASS TProgress
