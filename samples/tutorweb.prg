...
function Main()
   ...
   local oweb := TWebview():New(ownd,20,0,320,416,"http://www.google.com")

  oTbr = TToolBar():New( oWnd )
   oTbr:AddButton( "back",    { || oweb:Goback() } )
   oTbr:AddButton( "reload",   { || oweb:Reload() } )
   oTbr:AddButton( "Forward", { || oweb:GoForward() } )
   oWnd:Activate()
