...
function Main()
   ...
   local oWnd := TWindow():New(), oSlider, oLabel
   
   oSlider = TSlider():New( oWnd, 100, 10, 300, 50 )
   oLabel = TLabel():New( oWnd, Str( oSlider:nValue ), 200, 100, 100, 50 )
   
   oSlider:bChanged = { | oSelf | oLabel:SetText( Str( oSelf:nValue ) ) } 
