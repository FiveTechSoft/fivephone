...
function Main()
   ...
   oBrw = TTableView():New( oWnd,0,20,,,{ "One", "Two", "Three", "Four", "Five" },,, 92 )

   oBrw:bSelect   = { | nRow, nSection | MsgInfo( nRow ) }
   oBrw:bCellBack = { || "cell.png" }
  oBrw:bCellImage = { || "fts.png" }
  // oBrw:SetBackClear()
   oBrw:bCell:={ |ocel| ocel }
   
   obrw:bcell:= {| ocel,o,ele |  ajustacel(ele)   }

   oWnd:Activate()
   
return nil

Function ajustacel(ele)
