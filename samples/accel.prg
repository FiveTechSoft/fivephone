...
function Main()
   ...
   oLabel = TLabel():New( oWnd, "i'm moving", 30, 10, 100, 30 )
   
   oAccel = TAcceleroMeter():New()
   oAccel:bShaking = { | oSelf, x, y | movi( oLabel, x, y ) }
   
   oWnd:Activate()
   
return nil    

Function movi( oLabel, x, y )
   local nTop, nCol
     
   nTop = oLabel:nTop() - y * 30
   if nTop > ScreenHeight() - oLabel:nHeight 
      nTop =  ScreenHeight() - oLabel:nHeight 
   elseif nTop < 0 
      nTop = 0 
   endif
   
   nCol = oLabel:nLeft() + x * 30  
   if nCol > ScreenWidth() - oLabel:nWidth 
      nCol =  ScreenWidth() - oLabel:nWidth 
   elseif nCol < 0 
      nCol = 0 
   endif
   
   oLabel:SetPos( nTop, nCol )
