...
METHOD New() CLASS TAcceleroMeter

   ::hWnd = CreateAcceleroMeter( Self )
   
//   AAdd( GetAllWin(), Self )
   
return Self

METHOD HandleEvent( nMsg, hSender, uParam1, uParam2 ) CLASS TAcceleroMeter

   do case
      case nMsg == WM_SHAKING
           if ! Empty( ::bShaking )
              Eval( ::bShaking, Self, GETACCELEROMETERx( ::hWnd ), GETACCELEROMETERy( ::hWnd ), GETACCELEROMETERz( ::hWnd ) )
           endif
   endcase
