...
CLASS TSwitch FROM TView
   ...
   METHOD New( oWnd )
   METHOD Resources( oWnd, idResource,bSetGet )
   
   METHOD Set( lOnOff ) INLINE SwitchOnOff( ::hWnd, lOnOff ) 
   
   METHOD GetValue() INLINE SwitchGetValue( ::hWnd )
   
   METHOD HandleEvent( nMsg, hSender, uParam1, uParam2 )
   
ENDCLASS

METHOD New( oWnd, nTop,nLeft,nWidth, nHeight, bSetGet ) CLASS TSwitch

   local lValue := .T.

   DEFAULT nTop := 79, nLeft := 199, nWidth := 150, nHeight := 9, bSetGet := bSETGET( lValue )
   
   ::lValue := Eval( bSetGet )
   ::bSetGet = bSetGet
  
   ::hWnd = CreateSwitch( oWnd:hwnd, nTop, nLeft, nWidth, nHeight ) 

   AAdd( GetAllWin(), Self )
        
return Self         

METHOD Resources( oWnd, idResource,bSetGet ) CLASS TSwitch
  
   local lValue := .T.

   DEFAULT bSetGet := bSETGET( lValue )
   
   ::lValue := Eval( bSetGet )
   ::bSetGet = bSetGet
  
   ::hWnd = GetControlResource( oWnd:hWnd,idResource )
    CREATESWITCHRESOURCES(::hWnd) 
...
METHOD HandleEvent( nMsg, hSender, uParam1, uParam2 ) CLASS TSwitch

   do case
      case nMsg == WM_VALUECHANGE
         ::lValue = Eval( ::bSetGet, ::GetValue() )
         if ! Empty( ::bChange )
            Eval( ::bChange, Self )
         endif
